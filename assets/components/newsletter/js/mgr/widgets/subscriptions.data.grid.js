Newsletter.grid.SubscriptionsData = function(config) {
    ...
    config.tbar = [{
        ...
        xtype       : 'textfield',
        name        : 'newsletter-filter-subscriptions-data-search',
        id          : 'newsletter-filter-subscriptions-data-search',
        emptyText   : _('search') + '...',
        listeners   : {
            'change'    : {
        ...
        id          : 'newsletter-filter-subscriptions-data-clear',
        text        : _('filter_clear'),
        listeners   : {
            'click'     : {
                fn          : this.clearFilter,
...
Ext.extend(Newsletter.grid.SubscriptionsData, MODx.grid.Grid, {
    ...
    clearFilter: function() {
        ...
        Ext.getCmp('newsletter-filter-subscriptions-data-search').reset();

        this.getBottomToolbar().changePage(1);
    },
    getMenu: function() {
        return [{
            text    : '<i class="x-menu-item-icon icon icon-edit"></i>' + _('newsletter.subscription_data_update'),
            handler : this.updateData,
            scope   : this
        }, '-', {
            text    : '<i class="x-menu-item-icon icon icon-times"></i>' + _('newsletter.subscription_data_remove'),
