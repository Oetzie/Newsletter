<?php
	...
	class NewsletterNewsletterSendImmediatelyProcessor extends modObjectUpdateProcessor {
		/**
		 * @acces public.
		 * @var String.
		 */
		...
		public function initialize() {
			...
			if (3 == $this->getProperty('newsletter_type')) {
		...
		public function beforeSave() {
			if (1 == $this->modx->getOption('site_status')) {
				if (null !== ($mail = $this->modx->getService('mail', 'mail.modPHPMailer'))) {
					if (true === ($status = $this->object->getSendStatus()) || $this->getProperty('test')) {
						...
						if (!$this->getProperty('test')) {
							$this->object->sent_status = 1;
							$this->object->send_repeat = (int) $this->object->send_repeat - 1;
							
							if (0 < $this->object->send_repeat) {
								$this->object->send_status = 1;
								$this->object->send_date = date('Y-m-d', strtotime('+'.$this->object->send_interval.' days'));
							} else {
								$this->object->send_status = 2;
								$this->object->send_date = date('Y-m-d');
							}
							
							$this->object->setSendDetail($this->modx, $emails);
						}
						...
						$this->failure($this->modx->lexicon('newsletter.newsletter_send_error_'.$status.'_desc'));
					}
				} else {
	...
	return 'NewsletterNewsletterSendImmediatelyProcessor';
